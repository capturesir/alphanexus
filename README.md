...
- **新聞數據層(可插拔)**:預設 Yahoo 聚合;設定 `NEWS_PROVIDER` + `NEWS_API_KEY` 即切換至 NewsAPI / Marketaux 等正規授權來源(架構不變)。
...
| `NEWS_PROVIDER` / `NEWS_API_KEY` | 切換至正規授權新聞 API(`newsapi` 或 `marketaux`) | 未設定 = Yahoo 聚合 |
